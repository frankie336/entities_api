...
class DockerManager:
    ...
    _COMPOSE_ENV_MAPPING = {
        ...
        'MYSQL_USER': (DEFAULT_DB_SERVICE_NAME, 'MYSQL_USER'), # User for the application
        'MYSQL_PASSWORD': (DEFAULT_DB_SERVICE_NAME, 'MYSQL_PASSWORD'), # Password for the application user
        ...
        'DISABLE_FIREJAIL': ('sandbox_api', 'DISABLE_FIREJAIL'), # Example if set in sandbox service
    }

    # Define keys that should always be generated using secrets module if not found elsewhere
    _GENERATED_SECRETS = ['SIGNED_URL_SECRET', 'API_KEY'] # SECRET_KEY is often app-specific, handle separately?

    # Define Tool IDs to be generated
    _GENERATED_TOOL_IDS = [
        'TOOL_CODE_INTERPRETER', 'TOOL_WEB_SEARCH',
        'TOOL_COMPUTER', 'TOOL_VECTOR_STORE_SEARCH'
    ]
    ...
    _ENV_STRUCTURE = {
        "Base URLs": [
            'ASSISTANTS_BASE_URL', 'SANDBOX_SERVER_URL', 'DOWNLOAD_BASE_URL'
        ],
        "Database Configuration": [
            'DATABASE_URL', 'SPECIAL_DB_URL', # Composite URLs first
            'MYSQL_ROOT_PASSWORD', 'MYSQL_DATABASE', 'MYSQL_USER', 'MYSQL_PASSWORD', # Individual components
        ...
        "Platform Settings": [
            'BASE_URL_HEALTH', 'SHELL_SERVER_URL', 'CODE_EXECUTION_URL',
            'SIGNED_URL_SECRET', 'DISABLE_FIREJAIL',
            'SECRET_KEY', # App's main secret key
        ],
        "SMB Client Configuration": [
            'SMBCLIENT_SERVER', 'SMBCLIENT_SHARE', 'SMBCLIENT_USERNAME',
            'SMBCLIENT_PASSWORD', 'SMBCLIENT_PORT'
        ],
        "Tool Identifiers": [
            'TOOL_CODE_INTERPRETER', 'TOOL_WEB_SEARCH',
            'TOOL_COMPUTER', 'TOOL_VECTOR_STORE_SEARCH'
    ...
    def _get_env_from_compose_service(self, service_name, env_var_name):
        ...
        if not self.compose_config: return None
        ...
        try:
            ...
            for port_mapping in ports:
                ...
                if len(parts) == 1:
                    ...
                    if cont_port == container_port_str:
                        self.log.debug(f"Found host port mapping: {host_port}:{cont_port} (with IP) for service {service_name}")
                        return host_port.strip()
    ...
    def _generate_dot_env_file(self):
        ...
        for env_key, (service_name, compose_key) in self._COMPOSE_ENV_MAPPING.items():
            ...
            if value is not None:
                ...
                generation_log[env_key] = f"Value sourced from {self._DOCKER_COMPOSE_FILE} ({service_name}/{compose_key})"
        ...
        for key in self._GENERATED_TOOL_IDS:
             if key not in env_values:
                env_values[key] = f"tool_{secrets.token_hex(10)}" # Made slightly longer
        ...
        db_host = env_values.get('MYSQL_HOST', DEFAULT_DB_SERVICE_NAME) # Use default service name if host missing
        db_port = env_values.get('MYSQL_PORT', DEFAULT_DB_CONTAINER_PORT) # Use default port if missing
        db_name = env_values.get('MYSQL_DATABASE')

        if all([db_user, db_pass is not None, db_host, db_port, db_name]): # Check all components exist
             ...
             if host_db_port:
                 ...
                 generation_log['SPECIAL_DB_URL'] = f"Constructed using host port ({host_db_port}) from compose"
             else:
                 log.warning(f"Could not find host port mapped to {DEFAULT_DB_CONTAINER_PORT} for service '{DEFAULT_DB_SERVICE_NAME}'. Cannot generate SPECIAL_DB_URL.")
                 generation_log['SPECIAL_DB_URL'] = "Skipped: Could not find host port mapping in compose"
                 # Optionally set a placeholder or omit the key entirely
                 # env_values['SPECIAL_DB_URL'] = "# NOT_CONFIGURED"
        else:
             log.warning("Missing one or more database components (user, password, host, port, name). Cannot construct DATABASE_URL/SPECIAL_DB_URL.")
        ...
        for section_name, keys_in_section in self._ENV_STRUCTURE.items():
            ...
            if not found_key_in_section:
                 env_lines.append(f"# (No variables configured for this section)") # Indicate empty section
    ...
    def _check_for_required_env_file(self):
        ...
        if not os.path.exists(self._ENV_FILE):
            ...
            self.log.info(f"[ENV SCAN] Required environment file '{self._ENV_FILE}' exists. NOT overwriting.")
    ...
    def _has_nvidia_support(self):
        ...
        if has_smi:
            try:
                ...
                self.log.debug("nvidia-smi command failed or not found. Assuming no NVIDIA GPU available via Docker.")
                return False
        return False
    ...
    def _start_ollama(self, cpu_only=True):
        ...
        if not cpu_only and self._has_nvidia_support():
            self.log.info("   Attempting to add --gpus=all flag for NVIDIA GPU support.")
            cmd.insert(2, "--gpus=all") # Insert after 'run'
        elif not cpu_only:
            self.log.warning("   GPU requested but NVIDIA support check failed or OS incompatible. Starting in CPU mode.")
        ...
        try:
            ...
            if self._is_container_running(container_name):
                ...
                self.log.error(f"❌ External Ollama container '{container_name}' failed to start or exited quickly. Checking logs...")
                try:
                    self._run_command(["docker", "logs", container_name], check=False, suppress_logs=False)
                except Exception as log_err:
                     self.log.error(f"Could not retrieve logs for '{container_name}': {log_err}")
                return False
        except Exception as e:
    ...
    def _ensure_ollama(self, opt_in=False, use_gpu=False):
        ...
        if os.path.exists("/.dockerenv") or os.environ.get("DOCKER_HOST"):
            self.log.warning("🛰 Running inside a container or connected to remote Docker daemon. Skipping external Ollama management.")
            return True
        if platform.system() == "Darwin":
            self.log.warning("⚠️ macOS detected. GPU passthrough for Docker Desktop is limited/experimental. Native Ollama app recommended for GPU.")
            # Continue with CPU Docker Ollama attempt, but warn.
            # use_gpu = False # Force CPU on Mac? Or let it try? Let's let it try for now.

        if not self._has_docker():
            self.log.error("❌ Docker command not found. Cannot manage external Ollama container.")
            return False
        ...
        if use_gpu and not gpu_available and platform.system() != "Darwin":
             self.log.warning("⚠️ GPU mode requested (--ollama-gpu), but NVIDIA support check failed. Proceeding in CPU mode.")
    ...
    def _run_docker_cache_diagnostics(self):
        ...
        try:
            ...
            if context_size_mb > 500: # Example threshold
                self.log.warning("Context size is large (> 500MB). Review .dockerignore for unnecessary files.")
            ...
            if ps_config.returncode != 0 or not ps_config.stdout.strip():
                self.log.warning("Could not reliably get services from 'docker compose config'. Diagnostics limited.")
                services = []
            else:
            ...
            if services:
                 self.log.info("Checking potential image history (using service names as guess)...")
                 for service in services:
                    image_name_guess = service
                    self.log.info(f"--- History for potential image '{image_name_guess}' ---")
                    try:
                        history_cmd = ["docker", "history", image_name_guess, "--no-trunc", "--format", "{{.ID}}: {{.Size | printf \"%-10s\"}} {{.CreatedBy}}"]
                        ...
                        if history.returncode == 0 and history.stdout.strip():
                            ...
                            self.log.info("No history found (image might not exist locally with this name).")
                        else:
                            log_func = self.log.warning if "No such image" in history.stderr else self.log.error
                            log_func(f"Could not get history for '{image_name_guess}'. Error:\n{history.stderr.strip()}")
                    except Exception as e:
                        self.log.error(f"Error running docker history for '{image_name_guess}': {e}")
            else:
                 self.log.info("Skipping image history check.")

        except Exception as e:
            self.log.error("An error occurred during Docker cache diagnostics: %s", e, exc_info=self.args.verbose)
        finally:
    ...
    def _handle_nuke(self):
        ...
        self.log.warning("This will: 1) Stop/remove this project's containers/volumes. 2) Prune ALL unused Docker data on your system (containers, networks, images, build cache, VOLUMES).")
        try:
        ...
        try:
            ...
            self.log.warning(f"Error during 'docker compose down': {e}. Continuing nuke.")
    ...
    def _handle_down(self):
        ...
        target_desc = f" for services: {', '.join(target_services)}" if target_services else " for all services in this project"
    ...
    def _handle_build(self):
        ...
        if self.args.parallel: build_cmd.append("--parallel")
    ...
    def _tag_images(self, tag, targeted_services=None):
        ...
        try:
            config_json_output = self._run_command(
                ...
                capture_output=True, check=True, text=True, suppress_logs=True
            ).stdout
            ...
            for service_name, service_config in services.items():
                ...
                if not image_name:
                    self.log.debug(f"Skipping service '{service_name}': No explicit 'image:' name defined.")
                    skipped_count +=1
                    continue
                ...
                for option in source_tag_options:
                     inspect_res = self._run_command(["docker", "image", "inspect", option], check=False, capture_output=True, suppress_logs=True)
                     if inspect_res.returncode == 0:
                         source_image_ref = option
                         break

                if not source_image_ref:
                     self.log.warning(f"Skipping tag for service '{service_name}': Could not find built source image matching '{source_tag_options}'.")
                     skipped_count += 1
                     continue
    ...
    def _handle_up(self):
        # ... (implementation from previous version) ...
        if not os.path.exists(self._ENV_FILE):
            self.log.error(f"Required environment file '{self._ENV_FILE}' is missing. Generate it first.")
            sys.exit(1)
        ...
        if self.args.build_before_up: up_cmd.append("--build")
    ...
    def parse_args():
        ...
        if args.clear_volumes: args.down = True
        ...
        if build_flags_set and args.mode == 'up' and not args.build_before_up:
             log.warning(f"Build flags used with --mode=up without --build-before-up/--build. Build flags ignored unless build step occurs.")
        ...
        if args.nuke and (args.mode != 'up' or args.down or args.clear_volumes or build_flags_set or args.with_ollama):
             log.debug("--nuke is exclusive. Other mode/build/down/ollama flags ignored.")
