...
async def run_sync_generator_in_thread(sync_gen_func, *args, **kwargs) -> AsyncGenerator[Any, None]:
    """
    ...
    def run_generator():
        try:
            ...
            # Signal completion
            future = asyncio.run_coroutine_threadsafe(queue.put(finished_sentinel), loop)
            try:
                future.result()
            except Exception as put_err:
    ...
    try:
        while True:
            ...
            if item is finished_sentinel:
                logging_utility.debug("Received finished sentinel from sync generator thread.")
                break
        ...
        try:
            await asyncio.wait_for(thread_task, timeout=5.0) # Wait briefly for cleanup
             ...
             logging_utility.warning("Timeout waiting for sync generator thread task to finish.")
...
async def completions(stream_request: ValidationInterface.StreamRequest): # Renamed for clarity
    """
    ...
    async def stream_generator():
        ...
        try:
            ...
            async for chunk in run_sync_generator_in_thread(
                general_handler_instance.process_conversation,
                **sync_gen_args
            ):
            # --- END CRITICAL CHANGE ---
                ...
                if isinstance(chunk, str):
                    try:
                        ...
                        logging_utility.warning(f"Received string chunk that wasn't valid JSON: '{chunk[:100]}...'")
                        chunk_data = {"type": "content", "content": chunk}
                elif isinstance(chunk, dict):
                    chunk_data = chunk
                else:
                    logging_utility.warning(f"Skipping unknown chunk type: {type(chunk)}")
