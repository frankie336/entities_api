...
def create_message(
    ...
    try:
        ...
        _push_to_redis(redis, new_message)
        return new_message
...
async def submit_tool_response(
    ...
    try:
        ...
        await _push_to_redis_async(redis, new_message)
        return new_message
    except HTTPException:
        raise
    except Exception as e:
...
def _push_to_redis(redis: Redis, message_obj):
    try:
        key = f"thread:{message_obj.thread_id}:history"
        data = message_obj.dict(exclude_unset=True)
        data.update(role=message_obj.role, content=message_obj.content or "")
        payload = json.dumps(data)

        redis.rpush(key, payload)
        redis.ltrim(key, -200, -1)
        logging_utility.debug(f"Pushed message {message_obj.id} to Redis '{key}'")
    except Exception as e:
        logging_utility.error(
            f"Redis push failed for {message_obj.id}: {e}", exc_info=True
        )


async def _push_to_redis_async(redis: Redis, message_obj):
    try:
        key = f"thread:{message_obj.thread_id}:history"
        data = message_obj.dict(exclude_unset=True)
        data.update(role=message_obj.role, content=message_obj.content or "")
