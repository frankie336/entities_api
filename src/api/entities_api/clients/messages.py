...
class MessagesClient:
    ...
    def create_message(
        ...
        try:
            validated_data = validation.MessageCreate(
                **message_data
    ...
    def retrieve_message(self, message_id: str) -> validation.MessageRead:
        ...
        try:
            ...
            validated_message = validation.MessageRead(
                **message
            )  # Validate data using Pydantic model
    ...
    def update_message(self, message_id: str, **updates) -> validation.MessageRead:
        ...
        try:
            validated_data = validation.MessageUpdate(
                **updates
            )  # Validate data using Pydantic model
            ...
            validated_response = validation.MessageRead(
                **updated_message
    ...
    def list_messages(
        ...
        try:
            ...
            validated_messages = [
                validation.MessageRead(**message) for message in messages
            ]  # Validate response using Pydantic model
            logging_utility.info("Retrieved %d messages", len(validated_messages))
            return [
                message.dict() for message in validated_messages
    ...
    def save_assistant_message_chunk(
        ...
        validation.MessageRead
    ]:  # Return MessageRead for final chunk, None for non-final chunks
        """
