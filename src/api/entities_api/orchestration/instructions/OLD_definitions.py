...
LEVEL_4_DEEP_RESEARCH_INSTRUCTIONS = {
    ...
    "L4_PARALLEL_EXECUTION": (
        ...
        "You must issue tool calls in batches wherever possible. "
        "Single tool calls per turn is a performance failure.\n\n"
        "**THE DEPENDENCY RULE (mandatory check before every turn):**\n"
        "Ask: 'Does Call B need data that Call A produces?'\n"
        "- If NO  → issue both in the SAME turn (parallel)\n"
        "- If YES → issue A first, wait for result, then issue B\n\n"
        "**✅ PARALLELIZABLE (issue in same turn):**\n"
        "- Multiple `perform_web_search` calls for different entities or queries\n"
        "  → Example: search NVIDIA revenue AND search AMD revenue simultaneously\n"
        "- Multiple `read_web_page` calls on different URLs you already have\n"
        "  → Example: read nvidia.com/ir AND read sec.gov/nvidia simultaneously\n"
        "- Multiple `search_web_page` calls on different URLs already in cache\n"
        "  → Example: Ctrl+F 'revenue' on page A AND Ctrl+F 'revenue' on page B\n\n"
        "**🔒 SEQUENTIAL ONLY (must wait for prior result):**\n"
        "- `perform_web_search` → THEN `read_web_page`\n"
        "  → Reason: you need the URL from search before you can read it\n"
        "- `read_web_page` → THEN `search_web_page` on that URL\n"
        "  → Reason: page must be in cache before Ctrl+F works on it\n"
        "- `read_web_page` → THEN `scroll_web_page`\n"
        "  → Reason: you must see 'Page 1 of X' before scrolling makes sense\n\n"
        "**TURN STRUCTURE FOR A COMPARATIVE TASK (e.g., 'NVIDIA vs AMD'):**\n\n"
        "  Turn 1 — PARALLEL DISCOVERY:\n"
        "    perform_web_search('NVIDIA FY2024 revenue investor relations')\n"
        "    perform_web_search('AMD CY2024 revenue investor relations')\n\n"
        "  Turn 2 — PARALLEL RECON (URLs now known from Turn 1):\n"
        "    read_web_page(nvidia_url)\n"
        "    read_web_page(amd_url)\n\n"
        "  Turn 3 — PARALLEL SNIPE (pages now in cache from Turn 2):\n"
        "    search_web_page(nvidia_url, query='fiscal 2024 revenue')\n"
        "    search_web_page(amd_url, query='full year 2024 revenue')\n\n"
        "  Turn 4 — STOP and report findings.\n\n"
        "**FORBIDDEN PATTERNS:**\n"
        "- ❌ Searching for NVIDIA, waiting, then searching for AMD\n"
        "- ❌ Reading a page, then reading another page in a separate turn "
        "when both URLs were known at the same time\n"
