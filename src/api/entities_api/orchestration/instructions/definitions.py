...
LEVEL_4_DEEP_RESEARCH_INSTRUCTIONS = {
    ...
    "L4_TOOL_CHEATSHEET": (
        ...
        "    - *After loading:* ALWAYS call `search_web_page` next. Do NOT scroll to find content.\n"
        "    - *Example:* `read_web_page(url='https://...', force_refresh=False)`\n\n"
        "3.  **`search_web_page(url: str, query: str)`** ← ALWAYS DO THIS BEFORE SCROLLING\n"
        ...
        "    - *If no match:* Try 2-3 synonym queries (e.g. 'Net Revenue' → 'Total Revenue' → 'Net Sales').\n"
        "    - *If still no match:* The data is NOT on this page. Get a new URL. Do NOT scroll.\n"
        "    - *Example:* `search_web_page(url='https://...', query='Net Revenue')`\n\n"
        "4.  **`scroll_web_page(url: str, page: int)`** ← LAST RESORT. NEARLY ALWAYS WRONG.\n"
        "    - *Permitted ONLY when ALL of the following are true:*\n"
        "      ✅ You have already run `search_web_page` on this URL and it returned a match.\n"
        "      ✅ The content is a sequential narrative (legal filing, earnings transcript, ordered report).\n"
        "      ✅ You need the surrounding context of a confirmed match, not the match itself.\n"
        "    - *HARD LIMIT:* Maximum **3 scroll calls per URL**. After 3 scrolls with no actionable "
        "result, you MUST stop, append ⚠️ to the Scratchpad, and report back to the Supervisor.\n"
        "    - *SCROLL GATE — ask yourself before every scroll call:*\n"
        "      ❓ Have I run `search_web_page` on this URL? → If NO: run it now instead of scrolling.\n"
        "      ❓ Did `search_web_page` return nothing? → If YES: this page has no answer. Get a new URL.\n"
        "      ❓ Have I already scrolled this URL 3+ times? → If YES: STOP. Append ⚠️ immediately.\n"
        "    - *FORBIDDEN uses:*\n"
        "      ❌ Keyword hunting ('maybe it's on the next page').\n"
        "      ❌ As a fallback when `search_web_page` returns no match.\n"
        "      ❌ On any URL where `search_web_page` has not been run first.\n"
        "      ❌ More than 3 times on the same URL.\n"
        "    - *If you are past page 2 and still have not found the target: you are doom-scrolling. "
        "STOP. Switch to `perform_web_search` with a new query and a different source.*\n"
