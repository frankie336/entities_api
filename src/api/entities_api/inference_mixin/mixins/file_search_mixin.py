...
# ➊  Admin-scoped SDK client (same as before)
client = Entity(
...
class FileSearchMixin:
    ...
    def handle_file_search(
        ...
        try:
            # ── 2. Resolve *the* file_search vector-store for this user ──
            ...
            search_results = client.vectors.unattended_file_search(
                ...
                vector_store_host="qdrant",  # internal network host
            )
            ...
            # ── 4. Ship results back to the assistant thread ─────────────
        ...
        # ── 5. Failure path – surface readable error info ────────────────
            ...
            # Try to push the error back to the thread so UX doesn’t hang
            try:
                self.submit_tool_output(
                    thread_id=thread_id,
                    assistant_id=assistant_id,
            ...
            # Propagate so upstream observers / metrics see the failure
            raise
