...
class DockerManager:
    ...
    _GENERATED_SECRETS = [
        ...
        "DEFAULT_SECRET_KEY",
        "SMBCLIENT_PASSWORD",
        "SEARXNG_SECRET_KEY",  # compose uses :? — must be generated
    ]
    ...
    # -------------------------------------------------------------------------
    # Variables where rotation on a live stack has data implications.
    # The configure command warns specifically about these.
    # -------------------------------------------------------------------------
    _DANGEROUS_ROTATION = {
        "MYSQL_PASSWORD",
        "MYSQL_ROOT_PASSWORD",
    ...
    def _prompt_user_required(self, env_values: dict, generation_log: dict):
        ...
        Behaviour:
        - If a key is already present in the shell environment it is inherited
          silently — the user is not re-prompted and the shell value wins.
          This covers CI/CD (export HF_TOKEN=...) and power users who pre-set
          values before running the manager.
        - Each remaining prompt explicitly states it is skippable with Enter.
        ...
        """
        # ── Step 1: inherit any values already in the shell environment ──
        inherited = {}
        needs_prompt = {}
        ...
        # ── Step 2: nothing left to prompt for ───────────────────────────
        if not needs_prompt:
            return

        # ── Step 3: non-interactive environment — skip all remaining ─────
        ...
        # ── Step 4: interactive prompts for remaining keys ────────────────
        typer.echo("\n" + "=" * 60)
    ...
    def _generate_dot_env_file(self):
        ...
        # Step 6 — interactively prompt for user-required values.
        # Inherits from shell environment first, only prompts for what remains.
...
def configure(
    ...
    After updating, the command prints the correct restart instructions based
    on which variables were changed — some require only --force-recreate,
    others require a full --down to take effect safely.
    ...
    typer.echo(f"✓ {len(updates)} variable(s) updated in .env: {', '.join(updates.keys())}")

    # ── Categorised restart advice based on what was actually changed ──
    dangerous = [k for k in updates if k in DockerManager._DANGEROUS_ROTATION]
    requires_down = [k for k in updates if k in DockerManager._REQUIRES_DOWN]

    if dangerous:
        typer.echo(
            f"\n⚠️  WARNING: {', '.join(dangerous)} cannot be safely rotated on a live stack.\n"
            "   The DB / Samba volume was initialised with the old credential.\n"
            "   To apply this change you must:\n"
            "     1. Back up your data first\n"
            "     2. entities-api docker-manager --down --clear-volumes\n"
            "     3. entities-api docker-manager --mode up\n"
            "   Skipping steps 2-3 will break the database connection."
        )
    elif requires_down:
        typer.echo(
            f"\n  Note: {', '.join(requires_down)} require a full restart to take effect:\n"
            "    entities-api docker-manager --down\n"
            "    entities-api docker-manager --mode up"
        )
    else:
        typer.echo(
            "\n  Restart the affected service(s) for changes to take effect:\n"
