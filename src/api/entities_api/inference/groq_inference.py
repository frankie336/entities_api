...
class CloudInference(BaseInference):
    ...
    def process_conversation(
        ...
        if not (
            ...
            logging_utility.debug(
                "New user message already exists in conversation history. Skipping append."
            )
        ...
        try:
            ...
            for chunk in stream_response:
                ...
                if current_run_status in ["cancelling", "cancelled"]:
                    logging_utility.info(
                        f"Run {run_id} is being cancelled or already cancelled, stopping response generation."
                    )
