...
class BaseInference(ABC):
    ...
    def _get_openai_client(self, api_key: Optional[str], base_url: Optional[str]) -> OpenAI:
        ...
        if api_key:
             ...
             logging_utility.debug(f"Getting/Creating OpenAI client for key: ****, url: {effective_base_url}")
             # ... creation logic ...
        else:
             # Return self.openai_client (the default one initialized in __init__)
             logging_utility.debug("Returning default OpenAI client.")
             if not self.openai_client: raise RuntimeError("Default client not initialized")
    ...
    def process_conversation(
        ...
        try:
            ...
            logging_utility.info(f"Run {run_id}: Starting stream via {type(streamer).__name__}")

            # Delegate the actual streaming and initial parsing
            stream_generator = streamer.stream(request_details=request_details, api_key=api_key)

            for standardized_chunk in stream_generator:
                if self.check_cancellation_flag():
                    logging_utility.warning(f"Run {run_id}: Cancellation detected during stream.")
                ...
                if chunk_type in ['content', 'code_chunk']: # Accumulate renderable content
                    accumulated_content += content
                elif chunk_type == 'error':
                     logging_utility.error(f"Run {run_id}: Error chunk received from streamer: {content}")
            ...
            logging_utility.error(f"Run {run_id}: Error during streamer execution: {stream_err}", exc_info=True)
            yield {'type': 'error', 'content': f"Stream processing failed: {stream_err}"}
            self.handle_error(accumulated_content, thread_id, assistant_id, run_id) # Save partial on error
            return # Stop

        # --- Post-Streaming Logic ---
        logging_utility.info(f"Run {run_id}: Stream finished. Accumulated content length: {len(accumulated_content)}")
        ...
        try:
            ...
            if accumulated_content:
                ...
                self.parse_and_set_function_calls(accumulated_content, accumulated_content)

            # Call the NON-NEGOTIABLE function call execution/processing
            if self.get_function_call_state():
                logging_utility.info(f"Run {run_id}: Processing detected function call state.")
                # Pass api_key if sub-calls might need it (like stream_function_call_output -> stream_hyperbolic)
                for func_chunk in self.process_function_calls(thread_id, run_id, assistant_id, model, api_key=api_key):
                     yield func_chunk # Yield chunks from function processing/response
            else:
                 # Finalize normally if no function call detected/processed
                 if assistant_reply and hasattr(self, 'finalize_conversation'): # Use accumulated?
                      self.finalize_conversation(accumulated_content, thread_id, assistant_id, run_id)
                 # Update status - careful not to override requires_action
                 if hasattr(self, 'run_service') and hasattr(validator, 'StatusEnum'):
                      current_status = self.run_service.retrieve_run(run_id).status # Check current status
                      if current_status not in [validator.StatusEnum.pending_action, validator.StatusEnum.cancelling, validator.StatusEnum.cancelled, validator.StatusEnum.failed, validator.StatusEnum.expired]:
                           self.run_service.update_run_status(run_id, validator.StatusEnum.completed)


        except Exception as post_stream_err:
             logging_utility.error(f"Run {run_id}: Error during post-stream processing: {post_stream_err}", exc_info=True)
             yield {'type': 'error', 'content': f"Post-stream processing failed: {post_stream_err}"}
             # Ensure status is set to failed
             if hasattr(self, 'run_service'): self.run_service.update_run_status(run_id, validator.StatusEnum.failed)
