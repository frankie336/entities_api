...
try:
    ...
    print(f"  2. The project root ('{project_root}') is correct and contains the necessary packages.")
    print(f"  3. Required packages ('projectdavid_common', 'entities_api') are installed.")
    sys.exit(1)
...
def setup_database(db_url: str) -> sessionmaker | None:
    ...
    if not db_url:
        ...
        print(f"Error: {ENV_VAR_DB_URL} environment variable not set or provided via --db-url.")
        return None
    try:
        ...
        with engine.connect() as connection:
            logging_utility.info(f"Successfully connected to database: {engine.url.database}")
        SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)
        logging_utility.info("Database session factory configured.")
        return SessionLocal
    except sqlalchemy_exc.OperationalError as e:
        logging_utility.error(f"Failed to connect to database: {e}", exc_info=False) # Less verbose log
        print(f"\nError: Could not connect to the database.")
        print(f"URL Used: {db_url[:15]}... (check full URL)") # Show partial URL for hint
        print(f"Details: {e}")
        print(f"Troubleshooting:")
        print(f"  - Is the database server running at the specified host/port?")
        print(f"  - Are the credentials (user/password) correct?")
        print(f"  - Is the database name ('{engine.url.database if 'engine' in locals() else 'N/A'}') correct?")
    ...
    try:
        ...
        if admin_user:
            ...
            if not admin_user.is_admin:
                logging_utility.warning(f"Existing user {admin_email} found but IS NOT admin. Setting is_admin=True.")
        ...
        logging_utility.error(f"Error finding or creating admin user '{admin_email}': {e}", exc_info=True)
        print(f"\nError: Failed during admin user lookup/creation: {e}")
        db.rollback() # Rollback any partial changes
        return None


def generate_and_save_key(db: Session, admin_user: User, key_name: str) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        logging_utility.error(f"Error generating or saving API key for user {admin_user.id}: {e}", exc_info=True)
...
def save_credentials(
    ...
    try:
        os.makedirs(os.path.dirname(creds_file_path), exist_ok=True) # Ensure directory exists
        ...
        logging_utility.error(f"Failed to write credentials file '{creds_file_path}': {file_err}", exc_info=True)
    ...
    try:
        os.makedirs(os.path.dirname(dotenv_path), exist_ok=True) # Ensure directory exists
        ...
        set_key(dotenv_path, "ADMIN_USER_ID", str(admin_user.id), quote_mode="always") # Ensure ID is string
        ...
        print(f"Info: Ensure '{os.path.basename(dotenv_path)}' and '{os.path.basename(creds_file_path)}' are in your .gitignore file!")

    except Exception as dotenv_err:
        logging_utility.error(f"Failed to update .env file '{dotenv_path}': {dotenv_err}", exc_info=True)
        print(f"\nWarning: Failed to update .env file at {dotenv_path}: {dotenv_err}")

def print_key_to_console(user: User, key_prefix: str, plain_key: str, creds_filepath: str, dotenv_filepath: str):
...
def parse_arguments():
    ...
    dotenv_path = find_dotenv(filename=DEFAULT_DOTENV_FILENAME, raise_error_if_not_found=False)
    ...
    if not args.db_url:
         parser.error(f"Database URL is required. Please set the {ENV_VAR_DB_URL} environment variable or use the --db-url argument.")
...
def run_bootstrap(args):
    ...
    try:
        ...
        if plain_text_key and key_prefix:
             save_credentials(plain_text_key, key_prefix, admin_user, args.creds_file, args.dotenv_path)
             print_key_to_console(admin_user, key_prefix, plain_text_key, args.creds_file, args.dotenv_path)
...
if __name__ == "__main__":
    ...
    logging_utility.info(f"Running with arguments: Email='{args.email}', DB URL='{args.db_url[:15]}...', Output Files='{args.creds_file}', '{args.dotenv_path}'")

    # --- Pre-run Checks (Write Permissions) ---
    for path in [args.creds_file, args.dotenv_path]:
        target_dir = os.path.dirname(path)
        if not os.path.exists(target_dir):
            print(f"Info: Directory '{target_dir}' for output file '{os.path.basename(path)}' does not exist. Will attempt to create.")
            # Attempting creation is handled within save functions now
        elif not os.access(target_dir, os.W_OK):
            print(f"Warning: Write permission might be missing for directory: '{target_dir}'. Saving '{os.path.basename(path)}' might fail.")
            logging_utility.warning(f"Potential write permission issue for directory: '{target_dir}'")
